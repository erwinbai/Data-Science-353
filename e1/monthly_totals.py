...
# due to changing the function paramater, I had to import monthly_totals_hint for loops to run 
...
def pivot_months_pandas(data):
    ...
    data_group = data.groupby(['name','month'])
    
    monthly_num = data_group.aggregate('sum').reset_index()
    monthly = monthly_num.pivot(index='name', columns='month', values='precipitation')
    
    count_num = data_group.aggregate('count').reset_index()
